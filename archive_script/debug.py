...
class AtlasMaskGenerator:
    def __init__(self, atlas_type, fmri_path, output_dir, **atlas_kwargs):
        """
    ...
    def generate_masks(self, target_labels,t_r, mask_filename_prefix="mask"):
        """
        ...
        resampled_mask = image.resample_to_img(mask, self.img, interpolation="nearest")
...
t_r = 1
# Create instance for subcortical atlas (amygdala)
subcortical_mask_generator = AtlasMaskGenerator(
    atlas_type='sub-maxprob-thr25-1mm',
    fmri_path= functional_img,
    output_dir=output_dir
)
amygdala_masks = subcortical_mask_generator.generate_masks(
    target_labels=["amygdala"],
    t_r= t_r,
    mask_filename_prefix="mask"
)
print("Generated masks:", amygdala_masks)